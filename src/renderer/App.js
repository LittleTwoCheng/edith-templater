...
const DEFAULT_FIELDS = {
  ...
  check_labeled_age_grade: false,
  check_age_grade: true,
  check_client_specified_testing_age_grade: false,
  check_item_nos: true,
      ...
      <Page>
        ...
        <Form onSubmit={onSubmit}>
          ...
          <TextInputWithCheckbox
            ...
            checked={fields.check_client_specified_testing_age_grade}
            errors={errors}
            onChange={onChange}
            fullWidth
          />
          ...
          <TextInput
            ...
            autoComplete={useMemo(
              () => ({
                suggestions: dataSet.applicant.map(applicant => ({
            ...
            data={{
              trigger: reduceOnData(
                ...
                (accumulated, applicant, changedFields) => {
                  if (applicant.name === changedFields.applicant_name) {
                    return pipe(
                      resetIfApplicantNotMatch(dataSet.buyer, fields, "buyer"),
            ...
            autoComplete={useMemo(
              () => ({
                suggestions: dataSet.manufacturer
                  .filter(manufacturer => {
                    return (
                      ...
                      manufacturer.applicant === fields.applicant_name
                    );
                  })
                  .map(manufacturer => ({
                    label: manufacturer.name,
                ...
                noSuggestions: ({ inputValue }) =>
                  fields.applicant_name
                    ? `No Matching under Applicant "${fields.applicant_name}"`
              ...
              () => ({
                suggestions: dataSet.buyer
                  .filter(buyer => {
                    return (
                      !fields.applicant_name ||
                      buyer.applicant === fields.applicant_name
                    );
                  })
                  .map(buyer => ({
                    label: buyer.name,
          ...
          <TextInputWithCheckbox
              ...
              () => ({
                suggestions: dataSet.supplier
                  .filter(supplier => {
                    return (
                      !fields.applicant_name ||
                      supplier.applicant === fields.applicant_name
                    );
                  })
                  .map(supplier => ({
                    label: supplier.name,
            ...
            data={{
              trigger: pipe(
                syncCheckableState("supplier_name", ["supplier_address"]),
                reduceOnData(
                  dataSet.supplier,
                  (accumulated, supplier, changedFields) => {
                    if (supplier.name === changedFields.supplier_name) {
                      return {
                        ...accumulated,
                        check_supplier_address: true,
          ...
          <TextInputWithCheckbox
            ...
            onChange={onChange}
            data={{
              trigger: syncCheckableState("supplier_address", ["supplier_name"])
            }}
            fullWidth
          />
          <Divider />
          ...
          <Collapse in={involed_divisions.length > 0}>
            ...
            >
              <div style={{ marginTop: 16 }}>
                <Chips list={involed_divisions} variant="outlined" />
              </div>
            </FormControlWithLabel>
          </Collapse>
          <BooleanInput
